...
void performPerftTest()
{
	...
	{
		int fen_index;
		for (fen_index = 0; fen_index < 122; fen_index++)
		{
			getFENfromPerft(allPerftLines[fen_index], FEN);
			...
			score = negaMaxSearch_perft(&board, depth);
			
			if (!isBoardEqual(&board, &board2))
			{				
				printf("ERROR: BOARD NOT RESTORED!\n");
			}
			getDepthMoveCountfromPerft(allPerftLines[fen_index], depth, &correctMoveCount);

			if (lastDepthPerftCount != correctMoveCount)
			{
				printf("PERFT TEST FAILED!!\n");
				ERROR
			}
		}
		printf("All perft tests to depth %d succeeded!\n\n", depth);
	}
...
static int negaMaxSearch_perft(BOARD *board, const int depth)
{
	...
	for (i = 0; i < movelist.numberOfMoves; i++)
	{
		...
		{		
			...
			score = -negaMaxSearch_perft(board, depth - 1);
...
static void getFENfromPerft(const char perftLine[], char FEN[])
{
	...
	do
	{
		currChar = perftLine[count];
		FEN[count] = perftLine[count];
		count++;
	} while (currChar != ';' && count < 1000);
...
static void getDepthMoveCountfromPerft(const char perftLine[], const int depth,  int *moveCount)
{
	...
	*moveCount = 0;
	...
	{
		if (perftLine[count] == 'D' && perftLine[count + 1] == (depth + '0'))
		{
			done = true;
		}
		count++;
	...
	//read past any white lines
	while (perftLine[count] == ' ' && count < 1000)
	{
		count++;
	}
	...
	//Allow a maximum of 9999999999 moves
	if (!(digitsInNumber > 0 && digitsInNumber <= 10))
	{
		printf("");
	}
	ASSERT(digitsInNumber > 0 && digitsInNumber <= 10)

	int i;
	for (i = 1; i <= digitsInNumber; i++)
	{

		*moveCount += (int)(perftLine[count - i] - '0') * (int)(pow(10, i - 1) + 0.5);
	}
}